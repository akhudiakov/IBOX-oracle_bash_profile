...
DECLARE
  v_sid number;
  s sys.v$session%ROWTYPE;
  p sys.v$process%ROWTYPE;
BEGIN
  begin
    select sid into v_sid
    from   sys.v$process p, sys.v$session s
  ...
  select * into s from sys.v$session where sid  = v_sid;
  select * into p from sys.v$process where addr = s.paddr;
  ...
  for c1 in ( select * from sys.v$sqltext
  ...
  for c1 in ( select * from sys.v$session_wait where sid = s.sid) loop
    dbms_output.put_line(chr(9)||c1.state||': '||c1.event);
  end loop;

--  dbms_output.put_line('Connect Info:');
--  for c1 in ( select * from sys.v$session_connect_info where sid = s.sid) loop
